		...
		switch (p_item->caps_list[i].cap) {
			...
			case TUNNEL_MAP_E:
				...
				if ((p_item->caps_list[i].hal_id == SWAC_HAL) && hw_litepath_available
					...
					if (pret == PPA_SUCCESS) {
						g_pae_map_p_item_table[route.entry] = p_item;
						p_item->sess_hash = route.entry;
						p_item->routing_entry = route.entry;
						ppa_atomic_inc(&g_hw_session_cnt);
					} else {
						printk("PAE route FAILED for TSO!!!\n");
						/* FIXME, else can be removed */
					}
				}
				/*Create the session metadata (eg:template buffer) */
				if (ppa_hsel_add_sess_meta(&route, skb, tx_ifinfo, p_item->caps_list[i].hal_id)==PPA_SUCCESS) {
SKIP_ADD_SESS_META:
					if ((ret = ppa_hsel_add_routing_entry(&route, 0,
						...
						if ((p_item->flags & SESSION_ADDED_IN_SW) && (p_item->caps_list[i].hal_id == SWAC_HAL) &&
							...
							if (pret == PPA_SUCCESS) {
								...
								printk("PAE route FAILED for LRO!!!\n");
								/* FIXME */
							}
						}
#if IS_ENABLED(CONFIG_SOC_GRX500)
...
void ppa_hsel_del_routing_session(struct uc_session_node *p_item)
{
	...
	if ((p_item->flags & SESSION_ADDED_IN_HW) && (p_item->flags & SESSION_ADDED_IN_SW)) {
		...
		pret = ppa_hsel_del_routing_entry(&route, 0, PAE_HAL);
		p_item->flags &= ~SESSION_ADDED_IN_HW;
		ppa_atomic_dec(&g_hw_session_cnt);
	}
...
#if IS_ENABLED(CONFIG_PPA_MPE_IP97)
					if ((route.tnnl_info.tunnel_idx != ~0)  && (route.tnnl_info.tunnel_type != TUNNEL_TYPE_IPSEC) && !(p_item->flag2 & SESSION_FLAG2_VALID_IPSEC_TRANS))
#else
...
static bool is_any_member_of_gre_type(struct mc_session_node *p_item)
{
	int8_t i;
	struct netif_info *p_netif_info = NULL;
	bool found_gre_member = false;
	...
	for (i = 0; i < PPA_MAX_MC_IFS_NUM; i++) {
		...
		if (p_netif_info->flags & NETIF_GRE_TUNNEL) {
			ppa_netif_put(p_netif_info);
			found_gre_member = true;
			break;
		}
		ppa_netif_put(p_netif_info);
	}

	return found_gre_member;
}
/* ****************************************************************************************
...
#endif
	/* If any of the members in the mcast group has netif type
	GRE then that particular flow will take CPU Path */
	if (is_any_member_of_gre_type(p_item))
		return PPA_FAILURE;
	...
	for (i = 0; i < num_caps;) {
		...
		switch (p_item->caps_list[i].cap) {
			case SESS_MC_DS:
...
#endif
				/*In case of PAE HAL we need to add source mac of the packet to MAC table for the packet to be forwarded in aceelreation*/
				if (p_item->caps_list[i].hal_id == PAE_HAL) {
				...
				} else if (p_item->caps_list[i].hal_id == PPV4_HAL) {
				/*In case of LGM the HAL expects p_item->session_action to be NULL in case of calls from the multicast daemon*/
...
int32_t ppa_set_qos_meter(PPA_QOS_METER_CFG *meter_cfg)
{
	...
	flag = 0;
	ret = ppa_hsel_set_qos_meter_entry(meter_cfg, flag, TMU_HAL);
	if (ret == PPA_SUCCESS) {

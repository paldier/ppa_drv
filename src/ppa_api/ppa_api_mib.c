...
int32_t ppa_ioctl_get_iface_mib(unsigned int cmd, unsigned long arg, PPA_CMD_DATA *cmd_info)
{
	...
	if (res == PPA_SUCCESS) {
		cmd_info->if_info.acc_rx_ipv4 = ifinfo->hw_accel_stats.rx_pkts_ipv4
						+ ifinfo->sw_accel_stats.rx_pkts_ipv4;
		cmd_info->if_info.acc_tx_ipv4 = ifinfo->hw_accel_stats.tx_pkts_ipv4
						+ ifinfo->sw_accel_stats.tx_pkts_ipv4;
		cmd_info->if_info.acc_rx_ipv6 = ifinfo->hw_accel_stats.rx_pkts_ipv6
						+ ifinfo->sw_accel_stats.rx_pkts_ipv6;
		cmd_info->if_info.acc_tx_ipv6 = ifinfo->hw_accel_stats.tx_pkts_ipv6
						+ ifinfo->sw_accel_stats.tx_pkts_ipv6;

		if ( ppa_copy_to_user( (void *)arg, &cmd_info->if_info, sizeof(cmd_info->if_info)) != 0 )
			return PPA_FAILURE;

...
{
	uint8_t i;
	const char *tunnel_type [9] = {"NULL", "6RD", "DSLITE", "L2TP", "EOGRE", "6EOGRE", "IP0GRE", "IP60GRE", "IPSEC"};
...
static int proc_read_netif_seq_show(struct seq_file *seq, void *v)
{
	...
	for(i=0; i<ifinfo->sub_if_index; i++) {
		...
		if( i == ifinfo->sub_if_index -1 ) {
			seq_printf(seq, " with rx/tx:%llu/%llu(%x:%llu/%x:%llu)\n",

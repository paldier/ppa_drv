...
hdr_done:	
	...
	swa_dump_skb(p_swaHdr->hdr, swaHdr.tot_hdr_len, 0); 
		...
		if( PPA_SESSION_EXISTS == swa_get_session_from_skb(skb, pf, &p_item)) {	
			...
			if( IsSoftwareAccelerated(p_item->flags) 
				...
				if (IsPppoeSession(p_item->flags)) {
					if (p_item->flags & SESSION_ADDED_IN_HW) { /* Enabling TSO support for PPPoE */
					...
					} else { /* update ppp header with length of the ppp payload for SW path*/
						struct pppoe_hdr *ppphdr;
...
static int32_t swac_hal_generic_hook(PPA_GENERIC_HOOK_CMD cmd, void *buffer, uint32_t flag)
{
	...
	case PPA_GENERIC_HAL_INIT: /*init HAL*/ {
		...
		if((res = ppa_drv_register_cap(SESS_LOCAL_IN, 4, SWAC_HAL)) != PPA_SUCCESS)
			pr_err("ppa_drv_register_cap returned failure for capability SESS_LOCAL_IN!!!\n");

		if((res = ppa_drv_register_cap(SESS_LOCAL_OUT, 4, SWAC_HAL)) != PPA_SUCCESS)
			pr_err("ppa_drv_register_cap returned failure for capability SESS_LOCAL_OUT!!!\n");

		if((res = ppa_drv_register_cap(TUNNEL_6RD, 4, SWAC_HAL)) != PPA_SUCCESS)
